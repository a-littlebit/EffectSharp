            ...
            IncrementalValuesProvider<INamedTypeSymbol> modelProvider)
        {
            var methods = context.SyntaxProvider
                .ForAttributeWithMetadataName(
                    fullyQualifiedMetadataName: "EffectSharp.SourceGenerators.WatchAttribute",
                    predicate: static (node, _) => node is MethodDeclarationSyntax,
                    transform: static (ctx, _) => (IMethodSymbol)ctx.TargetSymbol)
                .Where(static m => m is not null)
                .Select(static (m, _) => (m, m.GetAttributeData("EffectSharp.SourceGenerators.WatchAttribute")))
